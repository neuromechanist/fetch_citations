...
def get_working_proxy(method: str = 'ScraperAPI', API_key: str = "paste-you-api-key-here"):
    success = False
    while not success:
        pg = ProxyGenerator()
        if method == 'ScraperAPI':
            ...
            success = pg.Luminati(usr='paste-your-username-here',
                                  passwd='paste-passwd-here', proxy_port=22225)
        else:
            # Try free proxies
            success = pg.FreeProxies()
...
def get_citations(dataset: str, num_cites: int) -> pd.DataFrame:
    ...
    for i in range(num_cites):
        try:
            ...
            print("Failed to connected, retrying one more time with a proxy ...")
            get_working_proxy()
            entry = scholarly.search_pubs(dataset, start_index=i)
